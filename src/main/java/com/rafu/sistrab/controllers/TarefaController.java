...
public class TarefaController {
  ...
  @PostMapping("multi")
  public ResponseEntity<List<TarefaDto>> saveMulti(
      @RequestBody @Valid @NotNull List<TarefaDto> dtos) {
    final var tarefas =
        service.saveAll(dtos.stream().map(mapper::convert).collect(Collectors.toList()));
    return ResponseEntity.ok()
        .body(tarefas.stream().map(mapper::toDto).collect(Collectors.toList()));
  }
