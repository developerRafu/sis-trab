...
public class PontoRestController {
  ...
  @PostMapping("/multi")
  public ResponseEntity<List<PontoResponse>> postMulti(
      @RequestBody final List<PontoRequest> request) {
    final var result =
        service.saveAll(request.stream().map(mapper::toPonto).collect(Collectors.toList()));
    return ResponseEntity.ok()
        .body(result.stream().map(mapper::toResponse).collect(Collectors.toList()));
  }
