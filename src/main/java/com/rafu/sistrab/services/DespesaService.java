...
public class DespesaService {
    ...
    public DespesaCalDto calc() {
        final var rendaTotal = rendaService.findAll().stream().map(Renda::getMontante).reduce(BigDecimal::add).orElse(BigDecimal.ZERO);
        final var despesaTotal = repository.findAll().stream().map(Despesa::getMontante).reduce(BigDecimal::add).orElse(BigDecimal.ZERO);
        final var economia = rendaTotal.subtract(despesaTotal);
        return DespesaCalDto.builder().despesasTotais(despesaTotal).rendaTotal(rendaTotal).economiaTotal(economia).build();
    }
}
